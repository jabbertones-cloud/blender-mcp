...
# ---- Sky Setup (v4: higher strength for side-view cameras) ----
try:
...
# ---- Sun Light (v4: NEW — directional sun for real shadows) ----
tod = '{time_of_day}'
if tod in ('day', 'overcast', 'dusk'):
    ...
    if tod == 'day':
        sun.data.energy = 3.0
        sun.data.angle = math.radians(0.5)  # Sharp shadows
        sun.rotation_euler = (math.radians(45), math.radians(15), math.radians(160))
    elif tod == 'overcast':
        sun.data.energy = 1.8
        sun.data.angle = math.radians(3.0)  # Soft diffuse shadows
