...
def _verify_postconditions(key: str, send_command, args: dict, result: dict) -> str | None:
    ...
    if key == "product.camera":
        ...
        # Verify requested args
        if "focal_length" in args:
            if "lens_mm" not in camera:
                return "camera lens_mm unobservable in diagnostics"
            if abs(camera["lens_mm"] - float(args["focal_length"])) > 0.1:
                return f"camera lens ({camera['lens_mm']}) does not match requested ({args['focal_length']})"

        if "use_dof" in args:
            if "dof_enabled" not in camera:
                return "camera dof_enabled unobservable in diagnostics"
...
def execute_canonical(key: str, arguments: dict, send_command, *, observe_visual: bool = True) -> dict:
    ...
    if response["status"] == "ok" and key in {"product.camera", "product.lighting"}:
        error_msg = _verify_postconditions(key, send_command, args, result)
        if error_msg:
            response["status"] = "postcondition_failed"
