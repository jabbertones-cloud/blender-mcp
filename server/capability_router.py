"""Deterministic capability routing for OpenClaw Blender MCP.

This module gives the MCP an explicit tool-selection layer instead of asking the
client model to reason over ~80 overlapping tools from names alone. It is kept
pure-Python so routing can be unit-tested without Blender.
"""
...
CAPABILITIES: Tuple[Capability, ...] = (
    Capability("blender_get_scene_info", "get_scene_info", "inspect", "understand the whole scene before editing", ("scene info", "inspect scene", "what is in", "analyze scene", "understand scene"), mutates_scene=False, priority=20),
    ...
    Capability("blender_create_object", "create_object", "model", "create a primitive, light, camera, or empty", ("create cube", "create sphere", "create cylinder", "create cone", "create torus", "add cube", "add sphere", "primitive", "new camera", "new light"), priority=14),
    ...
    Capability("blender_set_material", "set_material", "material", "apply a straightforward Principled material", ("material", "base color", "metallic", "roughness", "make it red", "make it blue", "glass material"), requires_object=True, negative=("nodes", "procedural material", "product material"), priority=8),
    ...
    Capability("blender_texture_bake", "texture_bake", "material", "bake materials/lighting/normals to textures", ("bake texture", "bake normal", "bake ambient occlusion", "texture bake"), requires_object=True, priority=28),
    Capability("blender_scene_lighting", "scene_lighting", "lighting", "set up general scene lighting", ("light the scene", "lighting setup", "three point lighting", "studio lighting", "rim light"), priority=13),
    Capability("blender_product_lighting", "product_lighting", "product-viz", "create product-photography lighting rigs", ("product lighting", "product photo", "product photography", "softbox", "hero product", "commercial product"), priority=30),
    Capability("blender_set_world", "set_world", "lighting", "change world/background/environment lighting", ("world background", "hdri", "environment", "background color", "world lighting"), priority=18),
    ...
    Capability("blender_product_camera", "product_camera", "product-viz", "compose product shots with product-specific camera controls", ("product camera", "hero shot", "packshot", "product angle", "product closeup"), priority=28),
    Capability("blender_camera_advanced", "camera_advanced", "camera", "perform advanced camera composition/lens/DOF operations", ("depth of field", "dof", "camera lens", "camera tracking", "frame object", "camera composition"), priority=20),
    Capability("blender_render", "render", "render", "render the current scene or animation", ("render", "final image", "render image", "render animation"), negative=("settings", "setup", "quality audit"), priority=12),
...
def _score(cap: Capability, normalized: str) -> Tuple[float, List[str]]:
    ...
    for phrase in cap.positive:
        ...
        if p and p in normalized:
            weight = 7.0 + min(len(p.split()), 4) * 2.0
            score += weight
    ...
    # Generic family cues are weaker than specific phrases.
...
def rank_capabilities(task: str, limit: int = 5, minimum_score: float = 7.0) -> List[Dict[str, Any]]:
    """Return ranked tool candidates for natural-language task text."""
...
def recommend(task: str, limit: int = 5) -> Dict[str, Any]:
    """Recommend the safest most-specific tool(s) and explain ambiguity."""
    candidates = rank_capabilities(task, limit=limit)
    if not candidates:
        return {
            "task": task,
            "decision": "inspect_then_plan",
            "primary": {
                "tool": "blender_get_scene_info",
                "command": "get_scene_info",
                "family": "inspect",
                "purpose": "understand the scene before choosing a mutation tool",
            },
            "alternatives": [],
            "confidence": 0.25,
            "reason": "No capability had a strong lexical match; do not guess a mutation tool.",
        }

    primary = candidates[0]
    second = candidates[1] if len(candidates) > 1 else None
    margin = primary["score"] - (second["score"] if second else 0.0)
    confidence = min(0.98, 0.55 + max(0.0, margin) / 30.0 + min(primary["score"], 35.0) / 100.0)
    ...
    return {
        ...
        "reason": (
            "Specific capability match with a clear margin."
            if decision == "use_primary"
            else "Several capabilities overlap; inspect context or ask for the missing target/detail before mutating."
        ),
    }


def plan_goal(goal: str, max_steps: int = 6, profile: str = "default") -> List[Dict[str, Any]]:
    """Create a routing-aware plan from a goal.

    The plan intentionally includes an initial read for mutation-heavy work and a
    final verification step. It does not fabricate tool arguments that require
    ...
    if profile != "power-user":
        steps.append({
            "description": "Inspect the current scene and establish object names/state",
            "tool_hint": "blender_get_scene_info",
            "command_hint": "get_scene_info",
            "kind": "inspect",
        })
    ...
    for rec in recs:
        ...
        steps.append({
            ...
            "tool_hint": tool,
            "command_hint": rec["command"],
            "kind": "mutate" if rec["mutates_scene"] else "inspect",
            "routing_score": rec["score"],
        })
        if len(steps) >= max_steps - 1:
            break

    if len(steps) == 1 and not recs:
        steps.append({
            "description": "Clarify the intended operation after scene inspection rather than guessing",
            "tool_hint": None,
            "command_hint": None,
            "kind": "clarify",
        })
    ...
    if profile != "power-user" and len(steps) < max_steps:
        steps.append({
            "description": "Verify the result against the requested outcome and spatial/visual constraints",
            "tool_hint": "blender_verify",
            "command_hint": None,
            "kind": "verify",
        })
...
def register_capability_router_tools(mcp_instance, send_command=None, format_result=None) -> List[str]:
    """Expose routing as read-only MCP tools so clients can ask before acting."""

    @mcp_instance.tool()
    async def blender_recommend_tools(task: str, limit: int = 5) -> Dict[str, Any]:
        """Choose Blender tools for a task before mutating the scene. Use this when tool choice is unclear or several Blender tools overlap."""
        return recommend(task, limit=limit)

    @mcp_instance.tool()
    async def blender_route_plan(goal: str, max_steps: int = 6, profile: str = "default") -> Dict[str, Any]:
        """Build a routing-aware Blender plan with concrete tool hints, inspection first, and verification last."""
