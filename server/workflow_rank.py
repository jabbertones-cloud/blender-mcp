...
_RULES = (
    ...
    (
        ...
        139.0,
        ["complete multi-view capture intent"],
        lambda t: any(x in t for x in ("multi view", "multiview", "front right top iso")),
        False,
    ),
    (
        "workflow.reference_attach",
        138.0,
        ["complete reference attach intent"],
        lambda t: any(x in t for x in ("attach reference", "reference image", "reference plate")),
        False,
    ),
    (
...
def workflow_match(query: str) -> List[dict]:
    ...
    matches: List[dict] = []
    for key, score, reason, pred, prepend in _RULES:
        if not pred(text):
            continue
        row = {
            "key": key,
