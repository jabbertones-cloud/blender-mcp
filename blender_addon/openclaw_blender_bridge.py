...
MAX_CONCURRENT_CLIENTS = int(os.environ.get("OPENCLAW_MAX_CONCURRENT_CLIENTS", "10"))
...
def handle_client_session(sock, semaphore=None):
    ...
    try:
        ...
        while True:
            ...
            chunk = sock.recv(min(65536, MAX_REQUEST_BYTES - len(raw) + 1))
            if not chunk:
                # EOF reached
                try:
            ...
            try:
                ...
                # Valid but incomplete UTF-8 string, wait for more data
                continue
        ...
        if data is None:
            ...
            try:
                ...
                err_resp = {"error": "Invalid request format"}
                if "id" in raw.decode("utf-8", errors="ignore"):
                    import re
                    m = re.search(r'"id"\s*:\s*([^,}\s]+)', raw.decode("utf-8", errors="ignore"))
                    if m:
                        val = m.group(1).strip('"')
                        if val:
                            err_resp["id"] = val
        ...
        try:
            ...
            err_resp = {"error": "Failed to serialize response"}
        ...
        try:
            error_msg = str(e)
            if not error_msg or isinstance(e, json.JSONDecodeError):
                error_msg = "Invalid request format"
            elif isinstance(e, TimeoutError):
                error_msg = "Client read timeout"
            elif "exceeds maximum" in error_msg:
                error_msg = "Request exceeds maximum allowed size"
            elif "UTF-8" in error_msg:
                error_msg = "Invalid UTF-8 sequence"
            elif "Invalid request format" in error_msg:
                error_msg = "Invalid request format"
            else:
                error_msg = "Internal server error"

            err_resp = {"error": error_msg}
            if data is not None and isinstance(data, dict) and "id" in data:
                err_resp["id"] = data["id"]
            elif "id" in raw.decode("utf-8", errors="ignore"):
                import re
                m = re.search(r'"id"\s*:\s*([^,}\s]+)', raw.decode("utf-8", errors="ignore"))
                if m:
                    val = m.group(1).strip('"')
...
def _accept_client(client_sock, client_addr, semaphore):
    """Helper to cleanly admit or reject a client based on semaphore."""
    if semaphore.acquire(blocking=False):
        threading.Thread(target=handle_client_session, args=(client_sock, semaphore), daemon=True).start()
        return True
    else:
        print(f"[OpenClaw Bridge] Rejecting client {client_addr}: Server busy")
        try:
...
def socket_server_thread():
    ...
    client_semaphore = threading.Semaphore(MAX_CONCURRENT_CLIENTS)
        ...
        try:
            ...
            for sock in readable:
                if sock is server_socket:
                    ...
                    _accept_client(client, addr, client_semaphore)
        except Exception as e:
            if running:
